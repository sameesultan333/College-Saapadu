// Shared request helper for every authenticated call the admin-dashboard
...
// App.js can drop back to the login screen. See CLAUDE.md section 33/34 —
 ...
 * Returns the raw Response (mirrors fetch) so existing callers that do
 * `const res = await fetch(...); return res.json();` only need to swap the
 * call site, not their response handling.
 */
...
export async function apiPatch(path: string, body?: unknown): Promise<any> {
  return parseJson(await apiFetch(path, { method: "PATCH", body: body !== undefined ? JSON.stringify(body) : undefined }));
}
...
export async function apiDelete(path: string): Promise<any> {
  return parseJson(await apiFetch(path, { method: "DELETE" }));
}
